...
class Person:
    ...
    def __init__(self,t,s):
        ...
        #каждый раз при создании игрока или капитана, он будет записан в массив footbalSociety, 
        #чтобы после чемпионата можно было проверить каждого игрока во всех прошедших играх
...
class Captain(Person):
    spec="Captain" #when searching for a player to define if it was a captain - когда позже будем искать игрока, 
                   #чтобы понять, что это был капитан и действовать соответственно
    win=0 #each Captain starts with zero amount of wins - у всех капитанов на старте ни одной победы
    def winUp(self):
        self.win=self.win+1 #win - победа одного из капитанов
    def getWins(self):
        return self.win
...
class footbalPlayer(Person):
    ...
    def followCaptain(self,Captain): #footbalPlayer plays in a team of one of the captains - 
                                     #игрок играет за одного из капитанов
...
h1=Captain(1, random.randint(7, 10)) #new Captains must have high playing skills - 
                                     #у капитанов обязан быть высокий уровень игры
...
while game:
    ...
    for i in range(10): #football team consists of 11 players, and the captain is one of them - 
                        #в команде 11 игроков, и капитан один из них
    ...
    if int(input("To continue enter 1, to exit enter any other number"))!=1: #just to stop the games -
                                                                             #для остановки матчей
        game=False
        print("Championship statistics!\nTeam 1 with captains skill level ",h1.getSkill()," won ",h1.getWins()," times!") 
        #championship results - выдаём результаты чемпионата по всем сыгранным матчам
        ...
        #пробуем найти игрока и показать уровень его скилла и за какую он команду
        #search starts after championship is ended, so we enter another loop for it - 
        #поиск начинается после окончания всех игр, и для этого отдельный цикл
        search=True
        while search:
            ans=input("Enter players id to know his skill and team he played in") 
            #введи ид игрока для поиска, либо exit для выхода из цикла
            if ans=="exit": #to exit this loop
                search=False
            elif int(ans)<=iD: #iD is global and unique for each created person type - 
                               #ИД глобальная переменная и уникальна для каждого из созданных игроков или капитанов
                for k in range(len(footballSociety)): #search in footbalSociety for a player - 
                                                      #ищем в том массиве нужного игрока
                    if footballSociety[k].getId()==int(ans):#and if id fits then this is right player - 
                                                            #и если ид совпадает, то это искомый игрок
                        if footballSociety[k].spec=="Captain": #if this is a captain, add one more line with wins- 
                                                #если это капитан, то добавляем ещё одну строку с числом побед
                            print("This person is a Captain and he won ",footballSociety[k].getWins()," times!")
                        footballSociety[k].getPerson()
            else:
                print("There is no person with this ID!") #если введено нечто некорректное, то пишем, что нет игрока с таким ИД
